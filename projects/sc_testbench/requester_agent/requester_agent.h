...
#include "requester_driver.h"

using namespace uvm;
using namespace sc_core;

class requester_agent : public uvm::uvm_agent {
 public:
  UVM_COMPONENT_UTILS(requester_agent);

  explicit requester_agent(uvm::uvm_component_name name)
      : uvm::uvm_agent(name) {}
  ...
  void build_phase(uvm_phase& phase) {
    ...
    requester_driver_h = requester_driver<my_req, my_rsp>::type_id::create(
        "requester_driver_h", this);
    sequencer_h =
  ...
  void connect_phase(uvm_phase& phase) {
    ...
    requester_driver_h->seq_item_port(sequencer_h->seq_item_export);
  }
  ...
  requester_driver<my_req, my_rsp>* requester_driver_h;
